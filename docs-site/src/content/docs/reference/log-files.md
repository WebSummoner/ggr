---
title: Log files
description: The structure of a Ggr log line, every status it can emit, and how to add custom labels.
sidebar:
  order: 2
---
...
Sometimes you may need to add some custom metadata like build number or branch
name to Selenium logs. Just add the following capability to your tests under
`ggr:options`:

```json title='Type: map, format: "<key>": "<value>"'
...
Such additional metadata in logs allows to better analyze respective Selenium
sessions.
